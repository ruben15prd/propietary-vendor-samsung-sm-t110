...
int cpufreq_joint_policy(struct cpufreq_policy *joint,
			 struct cpufreq_policy *policy) {
	enum cpufreq_governor_prio p, q;
	size_t i;

	if (policy->min > joint->max || policy->max < joint->min)
		return -1;
	if (policy->min > joint->min)
		joint->min = policy->min;
	if (policy->max < joint->max)
		joint->max = policy->max;
	if (!joint->governor[0])
		strcpy(joint->governor, policy->governor);
	else if (strcmp(joint->governor, policy->governor)) {
		for (i = 0; i < PRIO_CPUFREQ_MAX - 1; i++)
			if (!strcmp(joint->governor, supported_governors[i]))
				break;
		p = i;

		for (i = 0; i < PRIO_CPUFREQ_MAX - 1; i++)
			if (!strcmp(policy->governor, supported_governors[i]))
				break;
		q = i;
