...
public class IppResponse {
  ...
  private void setTextAttribute(byte tag) {
    ...
    if ((length != 0) && (_buf.remaining() >= length)) {
      ...
      attrValue.setTag(IppUtil.toHexWithMarker(tag));
      String tagName = getTagName(IppUtil.toHexWithMarker(tag));
          ...
          for (int z = 0; z < lll; z++) {
            ...
            if (attributeValue.getSetOfEnum() != null) {
              ...
              for (int w = 0; w < llll; w++) {
            	...
            	Log.d("cups4j", "IPPResponse.getEnumName(): " + "set-of-enum is null for attribute " + attributeName
