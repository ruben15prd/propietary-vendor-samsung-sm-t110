            ...
            int           verbose)	/* I - Update connecting-to-device state? */
{
  ...
  */
  ...
  if (number != current)
  {
    if (usb_set_configuration(printer->handle, number) < 0)
    {
      ...
      */
	  LOGI("usb-libusb.c/open_device Failed to set configuration");
      if (errno != EBUSY)
  ...
  */
  ...
  while (usb_claim_interface(printer->handle, number) < 0)
  {
	...
	LOGI("usb-libusb.c/open_device error 1");
    goto error;
  }
  ...
  */
    ...
    while (usb_set_altinterface(printer->handle, number) < 0)
    {
      if (errno != EBUSY)
        fprintf(stderr,
                ...
                "%s\n", number, printer->device->descriptor.idVendor,
	        printer->device->descriptor.idProduct, strerror(errno));
	  LOGI("usb-libusb.c/open_device error 2");
      goto error;
    }
  }
  ...
  if (verbose)
	...
	LOGI("usb-libusb.c/open_device success");
  return (0);
  ...
  */
  ...
  usb_close(printer->handle);
  printer->handle = NULL;
  LOGI("usb-libusb.c/open_device failed");
  return (-1);
}
...
int                                     /* O - Exit status */
usb_main(int  argc,                         /* I - Number of command-line arguments (6 or 7) */
     char *argv[])                      /* I - Command-line arguments */
{
...
/*
  ...
  */
                      ...
                      resource, sizeof(resource)) < HTTP_URI_OK)
  {
	...
	LOGI("STATE: usb_main : Error : No device URI found in argv[0] or in DEVICE_URI return 1");
    _cupsLangPrintFilter(stderr, "ERROR",
 ...
 if ((print_fd = open(argv[6], O_RDONLY)) < 0)
    {
		LOGI("usb_main Error : Unable to open print file");
  ...
  */
	LOGI("usb_main send the print file...");
  ...
  */
	LOGI("usb_main sending print file finished with status:%d",status);
  if (print_fd != 0)
    close(print_fd);
...
get_devices()
{
  ...
  char			device_id[1024],/* IEEE-1284 device ID */
			device_uri[1024];
					/* Device URI */
  usb_printer_t	printer;	/* Current printer */
	...
	for (bus = usb_get_busses(); bus; bus = bus->next)
	{
		...
		for (device = bus->devices; device; device = device->next)
		{
			...
			*/
			...
			if (!device->config || !device->descriptor.idVendor || !device->descriptor.idProduct){
				
				continue;
			}

			for (conf = 0, confptr = device->config;conf < device->descriptor.bNumConfigurations;conf ++, confptr ++)
			{
					   ...
					   if (!open_device(&printer,0))
						{
							...
							if (!get_device_id(&printer, device_id, sizeof(device_id)))
							{
								...
								LOGI("usb-libusb : get_devices : Printer found : device_uri : %s",device_uri);
								LOGI("usb-libusb : get_devices : Printer found : device_id : %s",device_id);
								/*
								* Get the device URI and make/model strings...
								*/
								backendGetMakeModel(device_id, make_model, sizeof(make_model));
								LOGI("usb-libusb : get_devices : Printer found : make_model : %s",make_model);
								
								device_info.device_uri = (char *) malloc(strlen(device_uri));
								memset(device_info.device_uri,0,sizeof(device_info.device_uri));
								strcpy(device_info.device_uri,device_uri);

								device_info.make_model = (char *) malloc(strlen(make_model));
								memset(device_info.make_model,0,sizeof(device_info.make_model));
								strcpy(device_info.make_model,make_model);

								device_info.device_id = (char *) malloc(strlen(device_id));
  ...
  * to print to...
  */
	LOGI("usb-libusb : get_devices : we haven't found a printer to print to...");
	return &device_info;;
}


jobjectArray Java_com_siso_app_genericprintservice_GPrintUsbManager_GetUsbDeviceList(JNIEnv *env, jobject thiz)
{
...
static JNINativeMethod sSearchMethods[] = {
	...
	{"GetUsbDeviceList", "()[Ljava/lang/String;",(void *) Java_com_siso_app_genericprintservice_GPrintUsbManager_GetUsbDeviceList}	
};
