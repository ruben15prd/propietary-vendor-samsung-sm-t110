...
//#define FINAL_CONTENT_TYPE "application/vnd.cups-postscript"
...
//extern void createJavaThread(const char* name, void (*start)(void *), void* arg);

typedef void (*update_state)(int,int); 
typedef void (*create_java_thread)(const char* name, void (*start)(void *), void* arg);

update_state update_job_state;
create_java_thread cr_thread_fun;
...
int ipp_main(int  argc,char *argv[], update_state job_state_fun, create_java_thread thread_fun)			
{
  int		i;			/* Looping var */
...
LOGI("ipp: device_uri = %s", device_uri);
...
//LOGI("ipp: state_reasons = %s", state_reasons);
...
LOGI("ipp: content_type = %s", content_type);
...
final_content_type="application/vnd.cups-postscript";
  ...
  */
  ...
  LOGI("ipp: hostname=%s port=%d device_uri=%s scheme=%s username=%s resource=%s", hostname, port, device_uri, scheme, username, resource);
...
/*http_addrlist_t *temp_c;
temp_c = addrlist;

while(temp_c->next!=NULL)
{
	LOGI("ipp: addrlist addr = %s ipv4 = %s", temp_c->addr, temp_c->addr);
	temp_c=temp_c->next;
}*/
  ...
  do 
  {
	...
	*/
        ...
        if (contimeout && (time(NULL) - start_time) > contimeout)
	{
	  ...
	  LOGI("ipp123: The printer is not responding.");
	  update_reasons(NULL, "-connecting-to-device");
	  return (CUPS_BACKEND_FAILED);
	}
	...
	switch (error)
	{
	  case EHOSTDOWN :
		  ...
		  LOGI("ipp: The printer may not exist or is unavailable at this time.");
	      break;
	  ...
	  case EHOSTUNREACH :
	      _cupsLangPrintFilter(stderr, "WARNING",
				   _("The printer is unreachable at this "
				     "time."));
		  LOGI("ipp: The printer is unreachable at this time.");
	      break;
	...
	_cupsLangPrintFilter(stderr, "ERROR",
	                     _("The printer is not responding."));
	LOGI("ipp: The printer is not responding. wait");
	wait_count++;
          if(wait_count >=10)
	{
	LOGI("IPP:Wait count finished: Exit");	
	return (CUPS_BACKEND_FAILED);
  ...
  do
  {
    ...
    */
        ...
        if (contimeout && (time(NULL) - start_time) > contimeout)
	{
	  _cupsLangPrintFilter(stderr, "ERROR",
	                       _("The printer is not responding."));
	LOGI("ipp: The printer is not responding--last log");
	return (CUPS_BACKEND_FAILED);
	}
    ...
    */
	                                  ...
	                                  IPP_TAG_KEYWORD)) != NULL)
    {
	  ...
	  LOGI("ipp: media-col-supported (%d values)",media_col_sup->num_values);
...
//      ppd = ppdOpenFile(getenv("PPD")); //changed@chandrashekar

//      ppd = ppdOpenFile("/mnt/sdcard/Generic-PostScript.ppd");
  ...
  if (format_sup != NULL)
  {
    ...
    {
      for (i = 0; i < format_sup->num_values; i ++)
	if (!_cups_strcasecmp("application/octet-stream", format_sup->values[i].string.text)) //changed@chandrashekar
	{
	  document_format = "application/octet-stream"; //changed@chandrashekar
	  break;
	}
    }
  }
  ...
  */
  ...
  (*cr_thread_fun)("monitor_printer", monitor_printer, &monitor);
  //createJavaThread("monitor_printer", monitor_printer, &monitor);
    ...
    */
    ...
    else
    {
      ...
      if (compatsize > 0)
      {
		...
		LOGI("ipp: Sending file using HTTP/1.1 chunking...");
      	}
    ...
    _cups_monitor_t *monitor)		/* I - Monitoring data */
{
  ...
  ipp_attribute_t *job_sheets;		/* job-media-sheets */
   ipp_attribute_t *job_sheets_completed;		/* job-media-sheets-completed */

  int update_sheet_count = 0;
  int job_state = -1;
  ...
  while (monitor->job_state < IPP_JOB_CANCELED && !job_canceled)
  {
	...
	*/
	...
	fprintf(stderr, "DEBUG: Get-Job-Attributes: %s (%s)\n",
		...
		if(!update_sheet_count)
		{
			if ((job_sheets = ippFindAttribute(response,
								 "job-impressions",
								 IPP_TAG_INTEGER)) != NULL){
					//update_JobState(1, job_sheets->values[0].integer);
					LOGI("ipp: monitor_printer job-impressions = %d",job_sheets->values[0].integer);
					update_sheet_count++;
				}
		}
	...
	if ((job_sheets_completed = ippFindAttribute(response,
				 "job-impressions-completed",
				 IPP_TAG_INTEGER)) != NULL)
	{
		LOGI("ipp: monitor_printer job-impressions-completed = %d",job_sheets_completed->values[0].integer);
		(*update_job_state)(job_state, job_sheets_completed->values[0].integer);
	}
	LOGI("ipp: monitor_printer job_state = %d",monitor->job_state);
	(*update_job_state)(job_state, 0);
...
report_printer_state(ipp_t *ipp)	/* I - IPP response */
{
  ...
  */
  ...
  if (ipp_supplies < 0)
  {
    ...
    if ((ppd = ppdOpenFile(ppdFilePath)) != NULL &&
