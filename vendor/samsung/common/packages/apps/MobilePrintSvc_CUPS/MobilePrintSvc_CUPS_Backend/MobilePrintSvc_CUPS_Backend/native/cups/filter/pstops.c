...
int					/* O - Exit status */
pstops_main(int  argc,				/* I - Number of command-line args */
...
//  }
  ...
  */
//  ppdFilePath = "/mnt/sdcard/Generic-PostScript.ppd";
  ppdFilePath = argv[6];
  psFilePath = argv[7];
	LOGI("filter/pstops.c argc=%d",argc);
  if (argc == 8){
    fp = cupsFileStdin();
  } else
  {
    ...
    */
    ...
    if ((fp = cupsFileOpen(argv[8], "r")) == NULL)
    {
		LOGI("Error : filter/pstops.c Cannot open input print file");
  ...
  */
  ...
  {
	LOGI("pstops_main: The print file is empty : return");
  ...
  */
  LOGI("pstops_main: ps file name: %s",psFilePath);
  int defout,outfd;
  defout = dup(1);
 // outfd = open("/mnt/sdcard/grvjojo",O_RDWR | O_CREAT);
...
jint Java_com_siso_app_genericprintservice_GPrintPsToPs_InvokePsToPs(JNIEnv* env, jobject thiz,jobjectArray stringArray)
{
	char *argv[100];
	...
	lpout = pstops_main(stringCount,argv);
	return lpout;
}

static JNINativeMethod sSearchMethods[] = {
     // name, signature, funcPtr 
	{"InvokePsToPs", "([Ljava/lang/String;)I",(void *) Java_com_siso_app_genericprintservice_GPrintPsToPs_InvokePsToPs}	
};
...
{	
	jclass clazz;
    clazz = (*env)->FindClass(env, "com/siso/app/genericprintservice/GPrintPsToPs");
    if (clazz == NULL) {
		return -1;
    }
