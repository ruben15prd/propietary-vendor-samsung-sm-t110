...
static int op_init(struct libusb_context *ctx)
{
	...
	if (!usbfs_path) {
		LOGI("libusb/os/linux_usbfs.c: could not find usbfs");
	...
	if (supports_flag_bulk_continuation == -1) {
		...
		if (supports_flag_bulk_continuation == -1) {
			...
			LOGI("libusb/os/linux_usbfs.c: op_init()--error checking for bulk continuation support");
			return LIBUSB_ERROR_OTHER;
		}
	}
	...
	struct discovered_devs **_discdevs, uint8_t busnum)
{
	...
	int r = 0;
	LOGI("libusb/os/linux_usbfs.c : usbfs_scan_busdir : usbfs_path :%s",usbfs_path);
	LOGI("libusb/os/linux_usbfs.c : usbfs_scan_busdir : busnum :%d",busnum);
	snprintf(dirpath, PATH_MAX, "%s/%03d", usbfs_path, busnum);
	LOGI("libusb/os/linux_usbfs.c : usbfs_scan_busdir : dirpath :%s",dirpath);
	usbi_dbg("%s", dirpath);
	dir = opendir(dirpath);
	if (!dir) {
		LOGI("libusb/os/linux_usbfs.c : usbfs_scan_busdir : opendir '%s' failed, errno=%d", dirpath, errno);
...
out:
	closedir(dir);
	LOGI("libusb/os/linux_usbfs.c : usbfs_scan_busdir : out : return r=%d",r);
	return r;
}
...
{
	struct dirent *entry;
	LOGI("libusb/os/linux_usbfs.c: usbfs_get_device_list() usbfs_path : %s",usbfs_path);
	DIR *buses = opendir(usbfs_path);
	struct discovered_devs *discdevs = *_discdevs;
	int r = 0;

	if (!buses) {
		LOGI("libusb/os/linux_usbfs.c: usbfs_get_device_list() : Error : opendir failed");
	...
	while ((entry = readdir(buses))) {
		...
		int busnum;
		LOGI("libusb/os/linux_usbfs.c: usbfs_get_device_list() : entry->d_name = %s",entry->d_name);
		if (entry->d_name[0] == '.')
			continue;

		busnum = atoi(entry->d_name);
		LOGI("libusb/os/linux_usbfs.c: usbfs_get_device_list() : busnum %d",busnum);
		if (busnum == 0) {
			LOGI("libusb/os/linux_usbfs.c: usbfs_get_device_list() : unknown dir entry %s", entry->d_name);
			usbi_dbg("unknown dir entry %s", entry->d_name);
			continue;
		}

		r = usbfs_scan_busdir(ctx, &discdevs_new, busnum);
		LOGI("libusb/os/linux_usbfs.c: usbfs_get_device_list() r=%d",r);
		if (r < 0)
			goto out;
	...
	int *usbfs_fallback)
{
	...
	if (sysfs_has_descriptors == -1) {
		...
		snprintf(filename, PATH_MAX, "%s/%s/descriptors", SYSFS_DEVICE_PATH,devname);
	...
	fd = fopen(filename, "r");
	if (!fd) {
		LOGI("libusb/os/linux_usbfs.c : sysfs_scan_device :open devnum failed, errno=%d", errno);
	...
	fclose(fd);
	if (r != 1) {
		LOGI("libusb/os/linux_usbfs.c : sysfs_scan_device : fscanf devnum returned %d, errno=%d", r, errno);
	...
	struct discovered_devs **_discdevs, int *usbfs_fallback)
{
	...
	if (!devices) {
		LOGI("libusb/os/linux_usbfs.c: sysfs_get_device_list() failed to opendir devices");
	...
	while ((entry = readdir(devices))) {
		...
		r = sysfs_scan_device(ctx, &discdevs_new, entry->d_name, usbfs_fallback);
