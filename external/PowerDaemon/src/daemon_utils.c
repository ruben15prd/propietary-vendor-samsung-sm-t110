...
/* int write_ppd_pid(const char *)
 *
 * Writes the pid file.
 *
 * Returns 0 on success, -1 otherwise.
 */
int write_ppd_pid(const char *pidfile)
{
	FILE *pid;
	struct stat sb;
	mode_t oldmask;
	...
	if (rc == 0) {
		...
		if (fscanf(pid, "%s", oldpid) == 1) {
			...
			/* if the file exists see if there's another powerdaemon process running */
			if (fd) {
				if (fscanf(fd, "%s", old_cmdline) == 1
						&& strstr(old_cmdline,"powerdaemon") != NULL) {
...
/* int clear_ppd_pid(const char *)
 *
 * Removes pid file
 *
 * Returns 0 on success, -1 otherwise.
 */
int clear_ppd_pid(const char *pidfile) {
