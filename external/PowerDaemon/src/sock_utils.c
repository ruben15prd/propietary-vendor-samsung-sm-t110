...
/* create a temporary directory for ppd socket.
...
char *create_temp_dir(char *buf, gid_t gid) {
	...
	if (mkdtemp(buf) == NULL) {
		...
		clog(LOG_WARNING, "Couldn't chown %s (%s).\n", buf, strerror(errno));
	}
	umask(oldmode);
...
void delete_temp_dir(const char *dirname) {
	...
	snprintf(filename, MAX_PATH_LEN, "%s%s", dirname, PPD_SOCKET);
...
int open_unix_sock(const char *dirname, gid_t gid) {
	...
	snprintf(sa.sun_path, 108 , "%s%s", dirname, PPD_SOCKET);
