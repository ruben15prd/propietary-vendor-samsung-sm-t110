...
class SSLContext {
 public:
  // SAMSUNG_CHANGE : cac_feature
  enum CACState {
    CAC_CONTEXT_NORMAL = 0,
	CAC_CONTEXT_ENGINE_LOADING = 1,
    CAC_CONTEXT_ENGINE_LOADED = 2,
    CAC_CONTEXT_CAC = 3,
  };
  static SSLContext* GetInstance() { return Singleton<SSLContext>::get(); }
  SSL_CTX* ssl_ctx() {
	LOG(INFO) << "[cac_debug_log][ssl_client_socket_openssl.cc] GetInstance() cac_state_: " << cac_state_ << " CAC_CONTEXT_CAC: " << CAC_CONTEXT_CAC;
    // SAMSUNG_CHANGE : cac_feature
    if (cac_state_ == CAC_CONTEXT_CAC) {
      LOG(INFO) << "ssl_cac_ctx_ is used";
	  if (ssl_cac_ctx_.get() != NULL)
	      return ssl_cac_ctx_.get();
	  // bai engine is not loaded. change state to normal
	  LOG(ERROR) << "can't use ssl_cac_ctx. use ssl_ctx as alternative";
	  cac_state_ = CAC_CONTEXT_NORMAL;
    }
    LOG(INFO) << "ssl_ctx_ is used";
    return ssl_ctx_.get(); 
  }
  ...
  // SAMSUNG_CHANGE : cac_feature
  bool setOpenSslContext(bool cac) {
	  LOG(INFO) << "setOpenSslContext - " << (cac?"true":"false");

	  if (cac_state_ == CAC_CONTEXT_NORMAL || cac_state_ == CAC_CONTEXT_ENGINE_LOADING) {
		  LOG(ERROR) << "setOpenSslContext() engined is not loaded! cac_state_(" << cac_state_ << ")";
		  return false;
	  }
	  if (cac) {
		  cac_state_ = CAC_CONTEXT_CAC;
	  } else {
		  cac_state_ = CAC_CONTEXT_ENGINE_LOADED;
	  }
	  return true;
  }

  bool registerCACEngine() {
	  LOG(INFO) << "[cac_debug_log][ssl_client_socket_openssl.cc] registerCACEngine() ";

	  if (cac_state_ == CAC_CONTEXT_ENGINE_LOADED ||
			  cac_state_ == CAC_CONTEXT_CAC) {
		  LOG(INFO) << "registerCACEngine() already registered";
		  return true;
	  }

	  cac_state_ = CAC_CONTEXT_ENGINE_LOADING;
	  int rv = registerEngine();
	  if (rv != ENG_OK) {
		  LOG(ERROR) << "fail to registerEngine() - " << rv;
		  return false;
	  }

	  SSL_CTX* ctx;
	  rv = setupCredentials(&ctx);
	  if (rv != ENG_OK) {
		  LOG(ERROR) << "fail to setupCredentials() - " << rv;
		  deregisterEngine();
		  return false;
	  }

	  SSL_CTX_sess_set_new_cb(ctx, NewSessionCallbackStatic);
	  SSL_CTX_sess_set_remove_cb(ctx, RemoveSessionCallbackStatic);
	  SSL_CTX_set_client_cert_cb(ssl_ctx_.get(), ClientCertCallback);
	  ssl_cac_ctx_.reset(ctx);
	  cac_state_ = CAC_CONTEXT_ENGINE_LOADED;
	  return true;
  }
  void deregisterCACEngine() {
	  if (cac_state_ == CAC_CONTEXT_NORMAL) {
		  LOG(INFO) << "deregisterCACEngine() CAC engine is not loaded";
		  return;
	  }

	  deregisterEngine();
	  ssl_cac_ctx_.reset(NULL);
	  cac_state_ = CAC_CONTEXT_NORMAL;
  }
  int getCACEngineState() {
	  LOG(INFO) << "[cac_debug_log][ssl_client_socket_openssl.cc] getCACEngineState() cac_state_: " << cac_state_;
	  return cac_state_;
  }
 private:
  ...
  // SAMSUNG_CHANGE : cac_feature
  crypto::ScopedOpenSSL<SSL_CTX, SSL_CTX_free> ssl_cac_ctx_;
...
// SAMSUNG_CHANGE : cac_feature
bool SSLClientSocketOpenSSL::setOpenSslContext(bool cac) {
  return SSLContext::GetInstance()->setOpenSslContext(cac);
}
bool SSLClientSocketOpenSSL::registerCACEngine() {
  return SSLContext::GetInstance()->registerCACEngine();
}
void SSLClientSocketOpenSSL::deregisterCACEngine() {
  SSLContext::GetInstance()->deregisterCACEngine();
}
int SSLClientSocketOpenSSL::getCACEngineState() {
	return SSLContext::GetInstance()->getCACEngineState();
}
