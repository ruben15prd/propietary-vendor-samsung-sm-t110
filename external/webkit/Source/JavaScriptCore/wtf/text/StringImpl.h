...
public:
    ...
    unsigned hash() const {
        //volatile unsigned length = m_length;
        //if(length>100000000)  // This is Hack But one way to reduce crash
	//   return m_hash;	
	if ((m_data!=NULL) && (m_length >0) && !m_hash)
		m_hash = StringHasher::computeHash(m_data, m_length);
	return m_hash;
   }
