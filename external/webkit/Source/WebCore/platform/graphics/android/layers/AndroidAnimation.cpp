...
#include "GLWebViewState.h" //Seems to ne SAMSUNG changes
...
bool AndroidAnimation::evaluate(LayerAndroid* layer, double time)
{
    ...
    if (!checkIterationsAndProgress(time, &progress)
        ...
        return false;
//FOllowign Change seems to be SAMSUNG change as not shown in opensource diff
    if (layer && layer->state() && layer->state()->isScrolling()) {
        return false;
    }
